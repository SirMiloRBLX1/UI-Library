...
--Lucide Icons https://raw.githubusercontent.com/SirMiloRBLX1/UI-Library/refs/heads/main/Icons.lua
local Icons = {}

local Success, Response = pcall(function()
	local RawCode = game:HttpGet("https://raw.githubusercontent.com/SirMiloRBLX1/UI-Library/refs/heads/main/Icons.lua")
	local LoadedModule = loadstring(RawCode)

	if typeof(LoadedModule) == "function" then
		Icons = LoadedModule()
	else
		error("Loaded data is not a valid function.")
	end
end)

if Success then
	print("Lucide Icon Has Been Loaded Successfully")
else
...
local function GetIcon(ID)
	...
	if Icons[ID] then
		return Icons[ID]
	elseif ID:match("^rbxassetid://%d+$") then
		return ID
	end
	return nil
end
...
CreateElement("ImageButton", function(ImageID)
	local ImageNew = Create("ImageButton", {
		Image = GetIcon(ImageID) or "",
		BackgroundTransparency = 1
	})
	return ImageNew
...
function OrionLib:MakeNotification(NotificationConfig)
	spawn(function()
		...
		NotificationConfig.Image = GetIcon(NotificationConfig.Image or "rbxassetid://4384403532")
...
function TabFunction:MakeTab(TabConfig)
	...
	TabConfig.Icon = GetIcon(TabConfig.Icon or "") -- 🔧 Use icon alias or rbxassetid
...
function ElementFunction:AddMultiDropdown(MultiDropdownConfig)
	...
	local SelectedLabel = AddThemeObject(SetProps(MakeElement("Label", "None", 13), {
		Size = UDim2.new(1, -40, 1, 0),
		Font = Enum.Font.Gotham,
		Name = "Selected",
		TextXAlignment = Enum.TextXAlignment.Right,
...
-- it waits for the ui to load
repeat task.wait() until gethui():FindFirstChild("Orion")

OrionLib:MakeNotification({
	Name = "Orion Library",
	Content = "Orion Has Loaded!",
	Time = 5
})

-- a notification like rayfield
local hasShownOrionNotification = false

if not hasShownOrionNotification then
    hasShownOrionNotification = true
